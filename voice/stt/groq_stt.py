...
logger = logging.getLogger("GroqSTT")

class GroqSTT(BaseSTT):
    def __init__(self, model_name: str = "whisper-large-v3"):
        # Map common Whisper model selection names to Groq-supported models
        if model_name in ("tiny", "base", "small", "medium", "large", "large-v3", "whisper-large-v3"):
    ...
    def _pcm_to_wav(self, pcm_data: bytes, sample_rate: int = 16000, channels: int = 1, bits_per_sample: int = 16) -> bytes:
        """Prepend a standard WAV header to raw PCM data so Groq can parse it as audio/wav."""
    ...
    async def transcribe(self, audio_data: bytes) -> str:
        """Upload and transcribe audio bytes using Groq's Whisper API."""
        logger.info("Transcribing audio using Groq Whisper API...")
        try:
            config = Container.resolve("config")
            
            # Resolve Groq API key: check environment first, then config.voice.api_key, then config.llm.api_key
            api_key = get_env("GROQ_API_KEY")
            if not api_key:
                api_key = config.voice.api_key
            if not api_key and config.llm.provider == "groq":
                api_key = config.llm.api_key

            if not api_key:
                logger.warning("Groq API key not configured. Returning empty transcription.")
                return ""
            ...
            # Language Part (force Spanish as default)
            body.append(f'--{boundary}'.encode('utf-8'))
            ...
            def _send_request():
                ...
                try:
                    ...
                    logger.error(f"Groq Whisper STT API error ({e.code}): {error_body}")
                    raise
                except Exception as e:
                    logger.error(f"Failed to communicate with Groq Whisper STT: {e}")
                    raise
