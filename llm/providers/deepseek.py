"""
Implementation of the DeepSeek LLM provider.
"""
import json
...
class DeepSeekProvider(BaseLLMProvider):
    ...
    async def generate(self, messages: List[Message], **kwargs) -> Message:
        if not self.api_key:
            return Message(
                role="assistant",
                content=f"[MOCK RESPONSE - DeepSeek API Key not configured]\nRespuesta simulada para DeepSeek '{self.model}':\nÚltimo mensaje: '{messages[-1].content}'"
            )
        ...
        def _send_request():
            ...
            try:
                ...
                raise Exception(f"Failed to communicate with DeepSeek: {str(e)}")
